...
function () {
  _createClass(ParseContext, null, [{
    ...
    value: function parseType(src, offset, inFlow) {
      switch (src[offset]) {
        case '*':
          return _Node.default.Type.ALIAS;

        case '>':
          return _Node.default.Type.BLOCK_FOLDED;

        case '|':
          return _Node.default.Type.BLOCK_LITERAL;

        case '{':
          return _Node.default.Type.FLOW_MAP;

        case '[':
          return _Node.default.Type.FLOW_SEQ;

        case '?':
          return !inFlow && _Node.default.atBlank(src, offset + 1) ? _Node.default.Type.MAP_KEY : _Node.default.Type.PLAIN;

        case ':':
          return !inFlow && _Node.default.atBlank(src, offset + 1) ? _Node.default.Type.MAP_VALUE : _Node.default.Type.PLAIN;

        case '-':
          return !inFlow && _Node.default.atBlank(src, offset + 1) ? _Node.default.Type.SEQ_ITEM : _Node.default.Type.PLAIN;

        case '"':
          return _Node.default.Type.QUOTE_DOUBLE;

        case "'":
          return _Node.default.Type.QUOTE_SINGLE;

        default:
          return _Node.default.Type.PLAIN;
      }
    }
  }]);
  ...
  function ParseContext() {
    ...
    Object.defineProperty(this, "parseNode", {
      configurable: true,
      enumerable: true,
      writable: true,
      value: function value(overlay, start) {
        ...
        switch (type) {
          case _Node.default.Type.BLOCK_FOLDED:
          case _Node.default.Type.BLOCK_LITERAL:
            node = new _BlockValue.default(type, props);
            break;

          case _Node.default.Type.FLOW_MAP:
          case _Node.default.Type.FLOW_SEQ:
            node = new _FlowCollection.default(type, props);
            break;

          case _Node.default.Type.MAP_KEY:
          case _Node.default.Type.MAP_VALUE:
          case _Node.default.Type.SEQ_ITEM:
            node = new _CollectionItem.default(type, props);
            break;

          case _Node.default.Type.COMMENT:
          case _Node.default.Type.PLAIN:
            node = new _PlainValue.default(type, props);
            break;

          default:
            node = new _Scalar.default(type, props);
        }
  ...
  _createClass(ParseContext, [{
    ...
    value: function parseProps(offset) {
      ...
      while (ch === _Node.default.Prop.ANCHOR || ch === _Node.default.Prop.COMMENT || ch === _Node.default.Prop.TAG || ch === '\n') {
        if (ch === '\n') {
          var lineStart = offset + 1;

          var inEnd = _Node.default.endOfIndent(src, lineStart);

          if (!_Node.default.nextNodeIsIndented(src[inEnd], inEnd - (lineStart + this.indent), true)) break;
          this.atLineStart = true;
          this.lineStart = lineStart;
          offset = inEnd;
        } else if (ch === _Node.default.Prop.COMMENT) {
